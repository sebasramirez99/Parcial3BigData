...
response1 = s3.get_object(Bucket="parcial3bigdatadata",Key=url)
data1 = response1['Body'].read()
print("hola", data1)
response = s3.get_object(Bucket="parcial3bigdatadata",Key=url1)
...
s3.upload_file("/tmp/tiempo.csv","parcial3bigdatadata",url11)
url12 = "headlines/final/periodico=Publimetro""/year="+str(year)+"/month="+str(month)+"/day="+str(day)+"/publimetro.csv"
s3.upload_file("/tmp/publimetro.csv","parcial3bigdatadata",url12)
